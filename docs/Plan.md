...
1. **[ ] P0** Build the shared core (`PageIssueFormatter`, `ScanIssuesExport`, recurrence
   analyzer) + fixture tests. Refactor `GetPageIssuesTool` onto it. *(No prod data needed.)*
2. **[ ] P0** `scans:export-issues {scan}` console command → trimmed JSON blob.
3. **[ ] P0** Reports feature — see [Plan-reports.md](Plan-reports.md).
4. **[ ] P1** REST endpoint(s) for the site-builder pull, once the JSON shape is proven by
   the report. Fix the token-naming landmine first.
5. **[ ] P2** Wire the report into a scheduled **quarterly** email (`->quarterly()`).
   Depends on the Forge scheduler firing every minute — it is currently set to **Nightly**,
   which must change to **Every Minute** or scheduled tasks won't run on cadence.
